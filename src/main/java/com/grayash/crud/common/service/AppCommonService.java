...
public class AppCommonService implements CodeConstant {
    ...
    public OTPResponse generateOTP(CommonRequest request){
        List<UserEntity> userDetails = repository.findByCustomerId(request.getCustomerId());
        OTPResponse response = new OTPResponse();
        if(userDetails!=null && userDetails.size()==1){
            OTPEntity otpEntity = new OTPEntity();
            otpEntity.setCustomerId(request.getCustomerId());
            otpEntity.setFlowId(request.getFlowId());
            otpEntity.setOTP(String.valueOf(CommonUtils.getOTP()));
            otpEntity.setPhoneNumber(userDetails.get(0).getPhoneNumber());
            otpEntity.setStatus(OTPStatus.GENERATED.name());
            otpEntity.setPhoneNumberCountry(userDetails.get(0).getCountryCode());
            ...
            throw new CustomerIdNotFoundException(request.getCustomerId());
        }
